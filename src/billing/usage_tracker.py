...
from saas.models import UsageRecord, Tenant
...
class UsageTracker:
    ...
    def get_usage_for_period(self, tenant_id: str, period: str) -> Dict[str, Any]:
        with self._session() as session:
            records = session.query(UsageRecord).filter_by(
                tenant_id=tenant_id, period_month=period
            ).all()
    ...
    def get_all_tenant_usage(self, period: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        with self._session() as session:
            ...
            ).filter_by(period_month=period).group_by(UsageRecord.tenant_id).all()
