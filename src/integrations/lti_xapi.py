...
class LTIIntegration:
    ...
    def extract_context(self, lti_launch: dict) -> dict:
        ...
        return {
            "course_id": lti_launch.get(
                "https://purl.imsglobal.org/spec/lti/claim/context", {}
            ).get("id"),
            "course_name": lti_launch.get(
                "https://purl.imsglobal.org/spec/lti/claim/context", {}
            ).get("label"),
    ...
    def roster_sync(self, context_id: str, access_token: str) -> list[str]:
        ...
        response = requests.get(f"https://api.lms.com/roster/{context_id}", headers=headers)
        users = response.json()
...
class XAPIIntegration:
    ...
    def send_convocatoria_created(self, convocatoria_id: str, organizer_email: str) -> bool:
        statement = XAPIStatement(
            ...
            object={
                "id": f"https://convocatorias.io/convocatorias/{convocatoria_id}",
                "definition": {"name": {"en-US": "Convocatoria Event"}},
            },
    ...
    def send_participation(self, convocatoria_id: str, attendee_email: str, attended: bool) -> bool:
        verb_id = (
            ...
            if attended
            else "http://adlnet.gov/expapi/verbs/absent"
        )
        statement = XAPIStatement(
